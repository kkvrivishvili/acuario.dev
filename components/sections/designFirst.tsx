...
import { TextHoverEffect } from '@/components/layout/footerEffect';
import { 
  Facebook,
  Instagram,
  Twitter,
...
const footerStyles = {
  wrapper: "relative w-full bg-background/80 backdrop-blur-[2px] py-16",
  container: "mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col items-center",
  nav: "flex items-center gap-8 mb-12",
  link: `
    text-sm text-muted-foreground/80 hover:text-foreground 
    transition-colors
  `,
  socialWrapper: "flex items-center gap-6 mb-12",
  socialLink: `
    text-muted-foreground/60 hover:text-foreground
    transition-colors
  `,
  effectContainer: "relative w-full h-24 mb-12", // Añadido margin bottom
  copyright: "text-sm text-muted-foreground/60",
};
    ...
    <footer className={footerStyles.wrapper}>
      <div className={footerStyles.container}>
        ...
        <nav className={footerStyles.nav}>
          ...
          <Link href="/jobs" className={footerStyles.link}>
            Jobs
          </Link>
          <Link href="/press" className={footerStyles.link}>
            Press
          </Link>
          <Link href="/accessibility" className={footerStyles.link}>
            Accessibility
          </Link>
          <Link href="/partners" className={footerStyles.link}>
            Partners
          </Link>
        </nav>
        ...
        {/* Effect Container */}
        <div className={footerStyles.effectContainer}>
          <TextHoverEffect />
        </div>
...
export const TextHoverEffect = ({
  ...
  const revealMaskId = useRef(`reveal-mask-${Math.random().toString(36).substr(2, 9)}`);
  ...
  return (
    <div 
      ref={containerRef}
      className={cn(
        "relative w-full min-h-[200px] h-full flex items-center justify-center p-8",
        className
      )}
      ...
      onMouseLeave={() => {
        setHovered(false);
        setCursor({ x: 0, y: 0 }); // Reset cursor position
      }}
    >
      <svg
        ref={svgRef}
      ...
      >
        <defs>
          ...
          <mask id={`mask-${revealMaskId.current}`}>
            <rect 
              x="0" 
              y="0" 
              width="100%" 
              height="100%" 
              fill={`url(#${revealMaskId.current})`}
        ...
        {/* Capa base del texto con efecto de outline */}
        <g 
        ...
        {/* Capa con efecto de gradiente y máscara */}
        <g
          fill="transparent"
          strokeWidth="0.5"
          stroke={`url(#${gradientId.current})`}
          mask={`url(#mask-${revealMaskId.current})`}
          style={{ opacity: hovered ? 1 : 0 }}
          className="transition-opacity duration-300"
        >
          {allPaths.map((path, index) => (
